...
with tab1:
    ...
    if st.session_state.participants.empty:
        ...
        for _, row in st.session_state.participants.iterrows():
    ...
    if not st.session_state.participants.empty:
        to_remove = st.selectbox(
            ...
            format_func=lambda pid: st.session_state.participants.loc[st.session_state.participants["id"] == pid, "nom"].values[0]
        )
